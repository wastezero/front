  ...
  let (params, _) =
    React.useState(() =>
      Request.{
        ...
        params:
          Some([
            ("restaurant_id", Js.Json.number(restaurantId |> float_of_int)),
          ]),
      }
    );

  let (grid, _, _) = BranchService.grid(~params, ());
    ...
    <div onClick=onClickedInside>
      <span className="rounded-md shadow-sm">
        <button
          ...
          ariaExpanded=true>
          {switch (value) {
           | Some(branch) => React.string(branch.address.street)
           | None => React.string({j|Choose branch|j})
           }}
